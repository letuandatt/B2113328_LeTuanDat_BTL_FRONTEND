      ...
      <table v-if="borrowings.length > 0" class="custom-table">
        ...
        <tbody>
          <tr v-for="(borrowing, index) in borrowings" :key="borrowing._id">
            <td>{{ index + 1 }}</td>
            <td>{{ borrowing.sach.ten }}</td>
            <td>{{ formatDate(borrowing.ngaymuon) }}</td>
            <td>{{ !borrowing.ngaytra ? "Chưa trả" : formatDate(borrowing.ngaytra) }}</td>
          </tr>
        </tbody>
      </table>
      <p v-else>Không có dữ liệu mượn sách cho độc giả này.</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchByDocGia() {
      if (!this.docGiaId) {
        alert("Vui lòng nhập ID độc giả.");
        return;
      }
      try {
